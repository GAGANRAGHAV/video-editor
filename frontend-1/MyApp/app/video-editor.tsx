...
export default function VideoEditorScreen() {
  ...
  useEffect(() => {
    if (status.isPlaying) {
      const interval = setInterval(() => {
        if (videoRef.current) {
          videoRef.current.getStatusAsync().then((videoStatus: any) => {
  ...
  const addOverlay = async () => {
    ...
    if (newOverlayType !== 'text') {
      try {
        ...
        Alert.alert('Error', 'Error selecting media');
        return;
      }
    }
  ...
  const updateOverlayTiming = (id: string, start: number, end: number) => {
    setOverlays(
        ...
        {selectedOverlayId && (
          <View style={styles.selectedOverlayEditor}>
            ...
            <Slider
              ...
              minimumValue={0}
              maximumValue={status.durationMillis ? status.durationMillis / 1000 : 100}
              value={overlays.find(o => o.id === selectedOverlayId)?.startTime || 0}
              onValueChange={(value) => {
            ...
            <Slider
              ...
              minimumValue={0}
              maximumValue={status.durationMillis ? status.durationMillis / 1000 : 100}
              value={overlays.find(o => o.id === selectedOverlayId)?.endTime || 5}
              onValueChange={(value) => {
                const overlay = overlays.find(o => o.id === selectedOverlayId);
                if (overlay) {
                  updateOverlayTiming(
